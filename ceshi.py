# -*- coding: utf-8 -*-
"""
Created on Mon May 28 17:02:57 2018
...
class Actor(object):
    def __init__(self, sess, n_features, action_bound, lr=0.0001):
        ...
        l1 = tf.layers.dense(
            inputs=self.s,
            units=150,  # number of hidden units
        ...
        bias_mu1 = tf.constant(6.)
        mu1 = tf.add(mu1, bias_mu1)

        bias_mu2 = tf.constant(1.5)
        mu2 = tf.add(mu2, bias_mu2)

        bias_mu3 = tf.constant(1.)
        ...
        self.mu1, self.sigma1 = tf.squeeze(mu1), tf.squeeze(sigma1 + 0.1)
        self.mu2, self.sigma2 = tf.squeeze(mu2), tf.squeeze(sigma2 + 0.1)
        self.mu3, self.sigma3 = tf.squeeze(mu3), tf.squeeze(sigma3 + 0.1)
        self.mu4, self.sigma4 = tf.squeeze(mu4), tf.squeeze(sigma4 + 0.1)
        ...
        self.multi_dist = tfd.MultivariateNormalDiag(
            loc=[self.mu1, self.mu2, self.mu3, self.mu4, self.mu5, self.mu6],
            scale_diag=[self.sigma1, self.sigma2, self.sigma3, self.sigma4, self.sigma5, self.sigma6]) #多元正态分布
        ...
        # tf.summary.histogram("l1_histogram", l1)
        # tf.summary.histogram("mu1_histogram", mu1)
        # tf.summary.histogram("sigma1_histogram", sigma1)
        # tf.summary.histogram("log_prob", log_prob)

    #        tf.summary.scalar("sigmal_scalar", sigma1)
...
def test():
    ...
    for episode in range(test_episode_num):
        # initial observation
        s = env.reset()
        #state_list = np.zeros([test_step_num, ])
...
actor = Actor(sess, n_features=N_S, lr=LR_A, action_bound=[-100, A_BOUND])
...
for i in range(MAX_EP_STEPS):
    ...
    while done:
        ...
        td_error = critic.learn(s, r/500, s_)  # gradient = grad[r + gamma * V(s_) - V(s)]
        print(td_error)
    ...
    # summary_l1 = sess.run(merge, feed_dict={
    #     actor.s: s_feed,
    #     actor.a: a
    # })
    # AC_writer.add_summary(summary_l1)
