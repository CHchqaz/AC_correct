...
class Actor(object):
    def __init__(self, sess, n_features, action_bound, lr=0.0001):
        ...
        bias_mu4 = tf.constant(1.)
        mu4 = tf.add(mu4, bias_mu4)

        bias_mu5=tf.constant(1.)
        mu5=tf.add(mu5,bias_mu5)

        bias_mu6=tf.constant(1.)
        mu6=tf.add(mu6,bias_mu6)
        # self.e = epsilon = tf.train.exponential_decay(2., global_step, 1000, 0.9)
        # self.mu, self.sigma = tf.squeeze(mu*2), tf.squeeze(sigma+0.1)
        self.mu1, self.sigma1 = tf.squeeze(mu1), tf.squeeze(sigma1 + 7)
        self.mu2, self.sigma2 = tf.squeeze(mu2), tf.squeeze(sigma2 + 7)
        self.mu3, self.sigma3 = tf.squeeze(mu3), tf.squeeze(sigma3 + 7)
        self.mu4, self.sigma4 = tf.squeeze(mu4), tf.squeeze(sigma4 + 8)
        #self.mu5, self.sigma5 = tf.squeeze(mu5 * 2), tf.squeeze(sigma5 + 0.1)
        #self.mu6, self.sigma6 = tf.squeeze(mu6 * 2), tf.squeeze(sigma6 + 0.1)
        self.mu5, self.sigma5 = tf.squeeze(mu5), tf.squeeze(sigma5 + 2.)
        self.mu6, self.sigma6 = tf.squeeze(mu6), tf.squeeze(sigma6 + 2.)
...
OUTPUT_GRAPH = False
MAX_EP_STEPS = 200 # default value = 200
GAMMA = 0.9
